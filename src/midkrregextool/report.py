...
def ask_output_path() -> Path:
    while True:
        name = input("Output file name with extension (e.g., results.txt): ").strip()
...
def format_bigram_results(a: Token, b: Token) -> str:
    if a.context is not None:
        ...
        contextwords[b.token_index - 1] = f"{contextwords[b.token_index-1]}>>"
        displayed_context = " ".join(contextwords)
        return f"{a.path}\t{a.source_id} {a.token_index}-{b.token_index} {a.is_note}\t{a.unicode_form} {b.unicode_form}\t{a.tagged_form} {b.tagged_form}\t{a.matched_part}\t{displayed_context}"
    else:
...
def write_hits(
    ...
    *,
    ...
    with open(path, "w", encoding=DEFAULT_OUTPUT_ENCODING, newline="\n") as f:
        f.write(
            f"FILE_PATH\tSOURCE\tTOKEN\t[TAGGED-FORM]\tMATCHED-PART\tCONTEXT\tNOTE\t# pattern={pattern!r} hits={len(hits)} purpose={purpose!r} note={note!r}\n"
        )
        if " " in pattern:
            for a, b in hits:
...
def maybe_save_hits(
    hits: list[tuple[Token, ...]], *, pattern: str, purpose: str | None = None
) -> None:
    if not hits:
