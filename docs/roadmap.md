...
(**parse → romanize → tag → analyze/annotate → search → report**),  
...
- ~~Split `run()` into mode-specific pipelines (`run_search`, `run_annotation`, `run_print_corpus`)~~
- ~~Clarify mode semantics: annotation mode triggered by `--annotation-mode`, print mode by `--print-corpus`~~
...
- ~~Separate analysis logic from interactive UI: `annotation.py` extracted from `tagger.py`~~
- ~~Consolidate annotation loop and candidate generation into `annotation.py`~~
...
- ~~Redesign annotation data format to store inflectional segmentation (`gold_morph` in JSONL)~~
- ~~Introduce morpheme-level lexicon built from annotation data (`infl_decomp`, `pos_to_allowed_morphemes`)~~
- ~~Implement inflection-internal segmenter (learned suffix decomposition from annotation data)~~

---

## Phase 4. Annotation Efficiency
...
- ~~Add timing instrumentation to annotation pipeline~~
...
- ~~Coverage display in annotation mode (covered/total tokens)~~
- ⬜ Collect and rank unanalyzed residual strings for targeted annotation
...
- ~~`--token-repr`: select `yale` vs `tagged_form` for search/annotation~~
